...
function getSpeciesByIds(...ids) { // ids que estão no teste
  const especiesFiltradas = species.filter((especie) => ids.includes(especie.id)); // se existir no elemento da iteração (especie), se inclui, um id que seja igual a ids (os que) estão colocados em teste - linha 9
